...
def parse_nmea_message(parsed_data):
    ...
    if msg_type == "GGA":
        ...
        print(f"⏰ Zeit: {parsed_data.payload[0]}")
        print(f"📍 Position: {parsed_data.payload[2]} {parsed_data.payload[3]}, {parsed_data.payload[4]} {parsed_data.payload[5]}")
        print(f"🚀 Geschwindigkeit (Knoten): {parsed_data.payload[6]}")
        print(f"🧭 Kurs: {parsed_data.payload[7]}")
        print(f"📅 Datum: {parsed_data.payload[8]}")
        ...
        time_utc = parsed_data.payload[5] if len(parsed_data.payload) > 5 else 'N/A'
        ...
        num_messages = parsed_data.payload[0] if len(parsed_data.payload) > 0 else 'N/A'
        message_num = parsed_data.payload[1] if len(parsed_data.payload) > 1 else 'N/A'
        num_sats_view = parsed_data.payload[2] if len(parsed_data.payload) > 2 else 'N/A'

        print(f"📡 GSV - Satelliten in Sicht: {num_sats_view}")
        print(f"📜 Nachricht {message_num} von {num_messages}")

        sat_data = []
        for i in range(4, len(parsed_data.payload), 4):  
            if len(parsed_data.payload) > i + 3:
                sat_info = {
                    "PRN": parsed_data.payload[i],
                    "Elevation": parsed_data.payload[i + 1],
                    "Azimuth": parsed_data.payload[i + 2],
                    "SNR": parsed_data.payload[i + 3] if parsed_data.payload[i + 3] != '' else 'N/A'
                }
        ...
        print(f"⚠ Unbekannter oder nicht unterstützter NMEA-Satz: {msg_type}")
