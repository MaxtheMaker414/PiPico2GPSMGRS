...
def parse_nmea_message(parsed_data):
    ...
    if msg_type == "GGA":
        ...
        altitude_unit = parsed_data.payload[10] if len(parsed_data.payload) > 10 else 'M'
        ...
        fix_mode = parsed_data.payload[1] if len(parsed_data.payload) > 1 else 'N/A'
        ...
        active_sats = [parsed_data.payload[i] for i in range(2, 14) if len(parsed_data.payload) > i and parsed_data.payload[i] not in ["", None]]
    ...
    while True:
        try:
            ...
            if uart1.any() > 0:
                ...
                if raw_data:
                    nmea_sentences = raw_data.decode("utf-8").strip().split("$")
                    for sentence in nmea_sentences:
                        if sentence:
                            sentence = f"${sentence}"
                            print(f"🔍 Einzelne NMEA-Nachricht empfangen: {sentence}")
                            try:
                                parsed_data = NMEAReader.parse(sentence)
                                if parsed_data:
